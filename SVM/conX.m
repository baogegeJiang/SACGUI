...
xp=zeros(length(i1),1);
xb=zeros(length(i1),1);
if fastCal==0
   for i=1:length(index0)
       ...
       for j=1:length(i1)
           xp(j,1)=norm(data(mod(index-i1(j):index-i2(j)-1,len)+1,3))/(i1(j)-i2(j)+1)^0.5;
           xb(j,1)=norm(data(mod(index+i2(j):index+i1(j)-1,len)+1,3))/(i1(j)-i2(j)+1)^0.5;
       end
